...
public class Blacksmith extends NPC {
	...
	public static void upgrade( Item item1, Item item2 ) {
		...
		Item first, second;
		int item1lvl = item1.level();
		int item2lvl = item2.level();
		if (item1 instanceof Weapon) {
			Weapon weapon1 = (Weapon)item1;
			item1lvl -= weapon1.augmentBonus ? 1 : 0;
		} else if(item1 instanceof Armor) {
			Armor armor1 = (Armor)item1;
			item1lvl -= armor1.augmentBonus ? 1 : 0;
		}

		if (item2 instanceof Weapon) {
			Weapon weapon2 = (Weapon)item2;
			item2lvl -= weapon2.augmentBonus ? 1 : 0;
		} else if(item2 instanceof Armor) {
			Armor armor2 = (Armor)item2;
			item2lvl -= armor2.augmentBonus ? 1 : 0;
		}
		...
		first.level(firstlvl+1); //prevents on-upgrade effects like enchant/glyph removal
