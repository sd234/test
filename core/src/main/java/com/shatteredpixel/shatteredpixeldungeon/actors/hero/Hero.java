...
public class Hero extends Char {
	...
	public int drRoll() {
		...
		if (block != null)  {
			int blockNum = block.blockingRoll();
			dr += blockNum;
			if (subClass == HeroSubClass.ENHANCER) {
			    int healNum =  Math.min(blockNum / 2, HT -HP);
				HP += healNum;
                sprite.emitter().start( Speck.factory( Speck.HEALING ), 0.4f, 1 );
                sprite.showStatus( CharSprite.POSITIVE, Integer.toString( healNum ) );
            }
		}
...
//		if (wep instanceof MeleeWeapon) dmg = wep.proc(this, enemy, dmg);
		if (dmg < 0) dmg = 0;
		...
		Char enemy = enemy();
		if (belongings.armor != null && subClass ==HeroSubClass.ENHANCER) {
			Armor armor = belongings.armor;
			if (armor.hasGlyph(Obfuscation.class, this) && enemy instanceof Mob && ((Mob) enemy).surprisedBy(this)) {
				dmg *= (int) Math.pow(1.05, armor.level());
			}
		}
	...
	public float speed() {
		...
		if (/*belongings.weapon != null &&*/buff(Kinetic.ConservedDamage.class) != null && subClass == HeroSubClass.ENHANCER) {
//			KindOfWeapon weapon = belongings.weapon;
//			if (((Weapon) (weapon)).enchantment != null) {
//				if ( ((Weapon) (weapon)).hasEnchant(Kinetic.class, this) ) {
					float addSpeed = buff(Kinetic.ConservedDamage.class).damageBonus();
					if (addSpeed > 5) {
						addSpeed -= 5;
						speed += .25f;
						if (addSpeed > 20) {
...
//					speed += buff(Kinetic.ConservedDamage.class).damageBonus();
//				}
//			}
		}
...
//				if (subClass == HeroSubClass.ENHANCER && belongings.armor.hasGlyph(Repulsion.class, this)) {
//					if (/*attacker.buff(Paralysis.class) != null && */Dungeon.level.heroFOV[ enemy.pos ]){
//						Buff.affect(enemy, Paralysis.class, 5f);
//						int dmg = (int)Math.ceil(enemy.buff(Paralysis.class).cooldown());
//						dmg *= Dungeon.depth / 5 + 1;
//						enemy.damage(dmg, this);
//						CellEmitter.get( enemy.pos - Dungeon.level.width() ).start(Speck.factory(Speck.ROCK), 0.07f, 10);
...
//				}
	...
	public void damage( int dmg, Object src ) {
		...
		if (belongings.armor != null && belongings.armor.hasGlyph(AntiMagic.class, this)
			...
			dmg -= AntiMagic.drRoll(belongings.armor.level() * (subClass == HeroSubClass.ENHANCER ? 3 : 2) / 2);
		}
	...
	public boolean isImmune(Class effect) {
		if (effect == Burning.class
			...
			if (subClass == HeroSubClass.ENHANCER) {
				int fireheal = Math.min(Dungeon.depth / 2, HT - HP);
				HP += fireheal;
				sprite.emitter().start( Speck.factory( Speck.HEALING ), 0.4f, 1 );
				sprite.showStatus( CharSprite.POSITIVE, Integer.toString( fireheal ) );
			}
			return true;
		}
