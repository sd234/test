...
public class StoneOfAugmentation extends InventoryStone {
	...
	public class WndAugment extends Window {
		...
		public WndAugment( final Item toAugment ) {
			...
			boolean isRunemage = curUser.heroClass == HeroClass.RUNEMAGE;
			boolean isRunemaster = curUser.subClass == HeroSubClass.RUNEMASTER;
					...
					if (((Weapon) toAugment).augment != aug){
						RedButton btnSpeed = new RedButton( Messages.get(this, aug.name()) ) {
							...
							protected void onClick() {
								hide();
								if (isRunemage && Random.Int(3) == 0) {
									StoneOfAugmentation.this.collect();
								}

								if (isRunemaster) {
									switch(((Weapon) toAugment).augment.name()) {
										case "DAMAGE":
											//toAugment.downgrade();
											((Weapon) toAugment).augmentBonus = false;
											break;
										case "NONE": case "SPEED": default:
											break;
									}
									switch (aug.name()) {
										case "DAMAGE":
											//toAugment.upgrade();
											((Weapon) toAugment).augmentBonus = true;
											break;
										case "NONE": case "SPEED": default:
											break;
									}
					...
					if (((Armor) toAugment).augment != aug){
						RedButton btnSpeed = new RedButton( Messages.get(this, aug.name()) ) {
							...
							protected void onClick() {
								...
								if (isRunemaster) {
									switch(((Armor) toAugment).augment.name()) {
										case "DEFENSE":
											((Armor) toAugment).augmentBonus = false;
											break;
										case "NONE": case "EVASION": default:
											break;
									}
									switch (aug.name()) {
										case "DEFENSE":
											//toAugment.enhancerUpgrade();
											((Armor) toAugment).augmentBonus = true;
											break;
										case "NONE": case "EVASION": default:
											break;
									}
