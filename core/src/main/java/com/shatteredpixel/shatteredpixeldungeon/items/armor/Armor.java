...
public class Armor extends EquipableItem {
	...
	public float evasionFactor( Char owner, float evasion ){
		...
		if (owner instanceof Hero){
			...
			if (hero.subClass == HeroSubClass.ENHANCER && hasGlyph(Flow.class, owner) && Dungeon.level.water[owner.pos]) {
			    evasion += 2 * level() + 5f;
            }
	...
	public float speedFactor( Char owner, float speed ){
		...
		if (hasGlyph(Swiftness.class, owner)) {
			...
			if (!enemyNear) speed *= (owner instanceof Hero && ((Hero) owner).subClass == HeroSubClass.ENHANCER) ? (1.5f + 0.08f * level()) : (1.2f + 0.04f * level());
	...
	public int STRReq(){
		...
		if ( (Dungeon.hero.subClass == HeroSubClass.RUNEMASTER && this.augment == Augment.EVASION)
                || (Dungeon.hero.subClass == HeroSubClass.ENHANCER) && hasGlyph(Swiftness.class, Dungeon.hero)) {
			strReq -= 2;
		}
		return strReq;
	}
