...
public class Elastic extends Weapon.Enchantment {
	...
	public int proc(Weapon weapon, Char attacker, Char defender, int damage ) {
		...
		int power = 2;
		float chance = (level + 1) / (float) (level + 5);//previous code is Random.Int( level + 5 ) >= 4

		if (isEnhancer) {
			power += level;
			chance += Dungeon.hero.lvl / 150f;
		}
		...
		if (Random.Float() < chance) {
			...
			WandOfBlastWave.throwChar(defender, trajectory, power);
		}
