...
public class Entanglement extends Glyph {
	...
	public int proc(Armor armor, Char attacker, final Char defender, final int damage ) {
		...
		if (Random.Int( 4 ) == 0) {
			...
			if (isEnhancer) {
				Buff.affect( defender, Barkskin.class).set( 3 + level, 1 );
				Buff.affect( defender, Earthroot.Armor.class ).level( 3 + 7 * level / 5);
				if (Dungeon.level.water[defender.pos] && Dungeon.level.water[attacker.pos] && !attacker.flying) {
					Buff.prolong(attacker, Roots.class, 3f);
				}
			} else {
