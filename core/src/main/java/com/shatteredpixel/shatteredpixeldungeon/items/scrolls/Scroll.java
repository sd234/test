...
public class Scroll extends Item {
	...
	public static boolean getStone = false;
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals( AC_READ )) {
			...
			if (hero.buff(MagicImmune.class) != null){
                ...
                getStone = true;//so that InventoryScroll can get rune stone correctly
				doRead();
				if (hero.heroClass == HeroClass.RUNEMAGE
                    && !(this instanceof ScrollOfIdentify || this instanceof ScrollOfRemoveCurse
                        || this instanceof  ScrollOfTransmutation || this instanceof  ScrollOfUpgrade)) {
					MagicStone magicStone = new MagicStone();
					if( !magicStone.collect() ) {
						Dungeon.level.drop( magicStone, hero.pos );
					}
				}

                if (curUser.subClass == HeroSubClass.RUNEMASTER) {
//                    boolean getStone = false;
                    getStone = false;//to avoid error when using InventoryScroll
                    Runestone stone = new StoneOfIntuition();
                    if (this instanceof ScrollOfLullaby) { stone = new StoneOfDeepenedSleep(); getStone = true;}
                    if (this instanceof ScrollOfMagicMapping) { stone = new StoneOfClairvoyance(); getStone = true;}
                    if (this instanceof ScrollOfMirrorImage) { stone = new StoneOfFlock(); getStone = true;}
                    if (this instanceof ScrollOfRage) { stone = new StoneOfAggression(); getStone = true;}
                    if (this instanceof ScrollOfRecharging) { stone = new StoneOfShock(); getStone = true;}
                    if (this instanceof ScrollOfRetribution) { stone = new StoneOfBlast(); getStone = true;}
                    if (this instanceof ScrollOfTeleportation) { stone = new StoneOfBlink(); getStone = true;}
                    if (this instanceof ScrollOfTerror) { stone = new StoneOfAffection(); getStone = true;}
                    if (getStone) { if(!stone.collect()) {
						Dungeon.level.drop( stone, hero.pos );
                    	}
					}
                    getStone = true;
                }
