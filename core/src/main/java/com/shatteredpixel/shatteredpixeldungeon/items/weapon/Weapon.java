...
abstract public class Weapon extends KindOfWeapon {
	...
	public boolean augmentBonus = false;
	...
	private static final String AUGMENT_BONUS 	= "augment_bonus";
	...
	public void storeInBundle( Bundle bundle ) {
		...
		bundle.put( AUGMENT_BONUS, augmentBonus );
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		augmentBonus = bundle.getBoolean( AUGMENT_BONUS );
	...
	public int reachFactor(Char owner) {
		return hasEnchant(Projecting.class, owner) ? RCH + (owner instanceof Hero && ((Hero)owner).subClass == HeroSubClass.ENHANCER ? 2 : 1) : RCH;
	}

	public int STRReq(){
		int strReq = STRReq(level());
		if (Dungeon.hero.subClass == HeroSubClass.RUNEMASTER && this.augment == Augment.SPEED) {
			strReq -= 2;
		}
		return strReq;
	}
	...
	@Override
	public int level() {
		return super.level() + (curseInfusionBonus ? 1 : 0) + (augmentBonus ? 1 : 0);
	}
	...
	public Item upgrade(boolean enchant ) {
		...
		if (enchant){
			...
			} else if (level() >= 4 && Random.Float(10) < Math.pow(2, level()-4)/* && curUser.subClass != HeroSubClass.ENHANCER*/){
				enchant(null);
			}
		}
	...
	public static abstract class Enchantment implements Bundlable {
		...
		public String name( String weaponName ) {
			...
			if (Dungeon.hero != null) {
				if (Dungeon.hero.subClass == HeroSubClass.ENHANCER) {
					return Messages.get(this, "name_enhancer", weaponName);
				}
			}
		...
		//this access my be useless before i change the MeleeWeapon.java's info
		//now curse may never use it
		public String desc() {
            ...
            if (Dungeon.hero != null) {
                if (Dungeon.hero.subClass == HeroSubClass.ENHANCER) {
                    return Messages.get(this, "desc_enhancer");
                }
            }
