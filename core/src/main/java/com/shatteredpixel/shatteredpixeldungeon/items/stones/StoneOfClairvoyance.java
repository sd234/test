...
public class StoneOfClairvoyance extends Runestone {
	...
	public void activate(final int cell) {
		//can't do that, or the game will bang!
		//DIST = curUser.heroClass != HeroClass.RUNEMAGE ? 12 : curUser.subClass != HeroSubClass.RUNEMASTER ? 15 : 20;
		...
		for (int y = Math.max(0, c.y - DIST); y <= Math.min(Dungeon.level.height()-1, c.y + DIST); y++) {
			...
			for (curr = left + y * Dungeon.level.width(); curr <= right + y * Dungeon.level.width(); curr++){
				...
				//FIXME Is these code useless?
				if (curUser.heroClass == HeroClass.RUNEMAGE) {
					int terr = Dungeon.level.map[curr];
					if ((Terrain.flags[terr] & Terrain.SECRET) != 0) {

						Dungeon.level.discover( curr );

						if (Dungeon.level.heroFOV[curr]) {
							GameScene.discoverTile( curr, terr );
						}
					}
				}
		...
		if (curUser.heroClass == HeroClass.RUNEMAGE) {
			if (curUser.subClass == HeroSubClass.RUNEMASTER) {
				Buff.affect(curUser, Light.class, curUser.lvl * 4f);
				Buff.affect( curUser, Awareness.class, 5f );
			} else {
				Buff.affect(curUser, Light.class, curUser.lvl * 3f);
				Buff.affect( curUser, Awareness.class, 1f );
			}
		}
