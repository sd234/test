...
public class Grim extends Weapon.Enchantment {
	...
	public int proc( Weapon weapon, Char attacker, Char defender, int damage ) {
		...
		float missingPercent = 0f;

		if (isEnhancer) {
			missingPercent = (attacker.HT - attacker.HP) / (float)attacker.HT;
			chanceMulti += missingPercent * (Dungeon.hero.lvl + Dungeon.depth) / 100f;
		}
		...
		if (Random.Float() < chance) {
			...
			if(missingPercent > .7f && defender.isImmune(Grim.class)) {
				defender.damage( Math.min(defender.HT / 4, defender.HP), attacker );
			}

			if(missingPercent > .7f && defender.properties().contains(Char.Property.BOSS)) {
				defender.damage( defender.HP / 4, attacker );
			}
