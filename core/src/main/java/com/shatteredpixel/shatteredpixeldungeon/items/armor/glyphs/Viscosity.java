...
public class Viscosity extends Glyph {
	...
	public int proc( Armor armor, Char attacker, Char defender, int damage ) {
		...
		if (isEnhancer) {
			if (realDamage > defender.HP) {
				int healSum = defender.HT / 10;
				defender.HP += healSum;
				attacker.sprite.emitter().start( Speck.factory( Speck.HEALING ), 0.4f, 1 );
				attacker.sprite.showStatus( CharSprite.POSITIVE, Integer.toString( healSum ) );
			}
			amount = (int)Math.ceil(realDamage - amount * .3f);
