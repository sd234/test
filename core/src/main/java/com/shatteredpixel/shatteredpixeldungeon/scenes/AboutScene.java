...
public class AboutScene extends PixelScene {
	...
	private static final String TTL_CLPX = "Cloud Pixel Dungeon";

	private static final String TXT_CLPX =
			"Design: 無孤な，火把烧门\n" +
			"Code & Graphics: 火把烧门";
	...
	public void create() {
		...
		final float colTop = (Camera.main.height / 2) - (SPDSettings.landscape() ? 30 : Camera.main.height * 2 / 5);
		final float shpxOffset = SPDSettings.landscape() ? colWidth : 0;
		final float wataOffset = SPDSettings.landscape() ? 2 * colWidth : 0;

		Image clpx = Icons.CLPX.get();
		clpx.x = (colWidth - clpx.width()) / 2;
		clpx.y = colTop;
		align(clpx);
		add( clpx );

		new Flare( 7, 64 ).color( 0xADD8Ef, true ).show( clpx, 0 ).angularSpeed = +20;

		RenderedTextBlock clpxtitle = renderTextBlock( TTL_CLPX, 8 );
		clpxtitle.hardlight( Window.CLPX_COLOR );
		add( clpxtitle );

		clpxtitle.setPos(
				(colWidth - clpxtitle.width()) / 2,
				clpx.y + clpx.height + 6
		);
		align(clpxtitle);

		RenderedTextBlock clpxtext = renderTextBlock( TXT_CLPX, 8 );
		clpxtext.maxWidth((int)Math.min(colWidth, 120));
		add( clpxtext );

		clpxtext.setPos((colWidth - clpxtext.width()) / 2, clpxtitle.bottom() + 12);
		align(clpxtext);

		//There is Shattered Pixel Dungeon

		Image shpx = Icons.SHPX.get();
		shpx.x = shpxOffset + (colWidth - shpx.width()) / 2;
		shpx.y = SPDSettings.landscape() ?
				colTop:
				clpxtext.top() + shpx.height + 10;
		align(shpx);
		add( shpx );
		...
		shpxtitle.setPos(
				shpxOffset + (colWidth - shpxtitle.width()) / 2,
		...
		shpxtext.setPos(shpxOffset + (colWidth - shpxtext.width()) / 2, shpxtitle.bottom() + 12);
		align(shpxtext);
