...
public class WndStartGame extends Window {
	...
	private static class HeroBtn extends Button {
		...
		HeroBtn ( HeroClass cl ){
			...
			} else if (cl == HeroClass.RUNEMAGE) {
				hero = new Image(Assets.RUNEMAGE, 0, 90, 12, 15);
			}
			add(hero);
				...
				if (cl != null) {
					...
					switch(cl){
						...
						case HUNTRESS:
							...
							break;
						case RUNEMAGE:
							heroItem.icon(new ItemSprite(ItemSpriteSheet.MAGIC_STONE, null));
							heroLoadout.icon(new ItemSprite(ItemSpriteSheet.RUNIC_SHORTWORD, null));
							heroMisc.icon(Icons.get(Icons.CHANGES));
							break;
					}
